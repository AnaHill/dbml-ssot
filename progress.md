...
- **Mermaid edge animation** (`e1@{ animate: true }`) in the interactive HTML — proposed, not implemented. Only for the `.html` output; GitHub's and Obsidian's bundled Mermaid may be too old for the `.md` one.
- **A PNG and clipboard export button in `lineage.html`**, deferred until after the repo is created. The catch: the highlight colors live in the page stylesheet, so a serialized SVG needs those `sel-*`/`dimmed` rules injected into a `<style>` inside it before being drawn to a canvas and downloaded. Exported images belong in `docs/img/`, not in gitignored `generated/`. Manual route until then: DevTools → right-click the `.diagram-zoom` element → Capture node screenshot.
- **Show/hide checkboxes for the `sources` and `orchestration` layers** in `lineage.html` — only the `TableGroup` layers can be hidden today, so the source nodes always stay on screen. The data side is already there: `graph_json()` tags every node with a layer and lists `sources`/`orchestration` among them ([lineage.py:277](scripts/lineage.py#L277)), and the JS hides by layer generically — the checkboxes are just rendered from `lin.groups` only ([lineage.py:698-701](scripts/lineage.py#L698-L701)). Worth checking when doing it: `sources` is a real Mermaid subgraph, so its `.cluster` element has to disappear with it, while the orchestration nodes are free-standing.
